...
TEST_CASE("Base128 encoding and decoding") {
    string input = "Hello, World!";
    string encoded, decoded;
    
    // Test encoding
    base128_encode(encoded, input);
    base128_decode(decoded, encoded);
    CHECK(decoded == input);
    
    // Test with empty string
    string empty_input = "";
    string empty_encoded, empty_decoded;
    base128_encode(empty_encoded, empty_input);
    CHECK(empty_encoded == "");
    base128_decode(empty_decoded, empty_encoded);
    CHECK(empty_decoded == "");
    
    // Test with binary data
    vector<uint8_t> binary_data = {0x00, 0x01, 0x02, 0xFF, 0xFE, 0xFD};
    string binary_encoded, binary_decoded;
    base128_encode(binary_encoded, binary_data);
    base128_decode(binary_decoded, binary_encoded);
